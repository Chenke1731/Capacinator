...
- **产品修复 ×2（P0 用户不可用级，2026-09-25 晚）**：① **/projects/new 表单三层断裂**——(a) 缺 project_sub_type_id 字段（API 强制配对校验 → 400）；(b) 幽灵 `location_id: ''`（state 有字段但表单从未渲染该控件）插入触发 FK 500（此前被 (a) 挡在更前面，修好 (a) 才暴露）；(c) onSuccess 取错信封层 `data.id` → `/projects/undefined`（从未到达，潜伏）。UI 从此完全无法建项目。修复：表单补 sub_type 联动下拉（换 type 重置）+ 服务端 nullableForeignKeys 两处加 location_id（'' → null）+ 客户端解信封。红线 test 4 已翻回 UI 驱动全链路。② **People 页无删除按钮**——mutation/handler 早已就绪（"reserved for future"），只缺按钮；补按钮 + confirm + 失败可见（场景分配 FK 拒删时 Alert 呈现服务端信息；基线分配随人级联删除，确认文案已注明）。红线 test 3 已翻回 UI 删除。
...
- **D11（P2）fixture profile-select 噪声**：authenticatedPage 的选人流程偶发卡 `#person-select` 30s（元素存在、冷启动时序问题），超时后"继续 anyway"不阻塞但拖慢测试。待查：people 查询在多 worker 并发冷启动下的挂起路径。**（2026-09-24 晚注：vite 代理 PORT 修复后 smoke 10/10 且 34.8s、scenario 全套两连绿未复现 D11——此前"冷启动慢"很可能部分归因于测试流量绕路 dev 后端；暂降级观察，复现再查。）**
